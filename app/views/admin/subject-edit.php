...
// Get subject ID
$subjectId = getParam('id');
if (!$subjectId) {
    setFlashMessage('Invalid subject ID', 'danger');
...
if (!$subject) {
    setFlashMessage('Subject not found', 'danger');
    header('Location: subjects.php');
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Check subject code uniqueness (excluding current subject)
    if (!empty($subjectCode)) {
        $stmt = $db->prepare("SELECT COUNT(*) FROM subjects WHERE subject_code = ? AND id != ?");
    ...
    if (empty($errors)) {
        try {
            $stmt = $db->prepare("UPDATE subjects SET subject_code = ?, subject_name = ?, grade_level = ?, track = ?, strand = ?, semester = ?, subject_type = ?, units = ?, description = ? WHERE id = ?");
            $stmt->execute([$subjectCode, $subjectName, $gradeLevel, $track, $strand, $semester, $subjectType, $units, $description, $subjectId]);
            
            // Audit log
            $stmt = $db->prepare("INSERT INTO audit_logs (user_id, action, table_name, record_id, ip_address) VALUES (?, 'update', 'subjects', ?, ?)");
            $stmt->execute([$_SESSION['user_id'], $subjectId, $_SERVER['REMOTE_ADDR']]);
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="mb-3">
                                ...
                                <select name="grade_level" class="form-select" required>
                                    ...
                                    <option value="11" <?php echo $subject['grade_level'] == '11' ? 'selected' : ''; ?>>Grade 11</option>
                                    <option value="12" <?php echo $subject['grade_level'] == '12' ? 'selected' : ''; ?>>Grade 12</option>
                                </select>
                            </div>
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="mb-3">
                                ...
                                <select name="semester" class="form-select" required>
                                    ...
                                    <option value="Year-round" <?php echo $subject['semester'] == 'Year-round' ? 'selected' : ''; ?>>Year-round</option>
                                </select>
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <select name="subject_type" class="form-select" required>
                                    ...
                                    <option value="Core" <?php echo $subject['subject_type'] == 'Core' ? 'selected' : ''; ?>>Core Subject</option>
                                    <option value="Applied" <?php echo $subject['subject_type'] == 'Applied' ? 'selected' : ''; ?>>Applied Track Subject</option>
                                    <option value="Specialized" <?php echo $subject['subject_type'] == 'Specialized' ? 'selected' : ''; ?>>Specialized Subject</option>
                                </select>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="number" name="units" class="form-control" min="0" step="0.5" value="<?php echo htmlspecialchars($subject['units']); ?>">
                            </div>
                        </div>
                    </div>
