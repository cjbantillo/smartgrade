...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Check year code uniqueness
    if (!empty($yearCode)) {
        $stmt = $db->prepare("SELECT COUNT(*) FROM school_years WHERE year_code = ?");
    ...
    if (empty($errors)) {
        try {
            ...
            $stmt = $db->prepare("INSERT INTO school_years (year_code, year_start, year_end, is_active) VALUES (?, ?, ?, ?)");
            $stmt->execute([$yearCode, $yearStart, $yearEnd, $setActive]);
            $yearId = $db->lastInsertId();
            
            // Audit log
            $stmt = $db->prepare("INSERT INTO audit_logs (user_id, action, table_name, record_id, ip_address) VALUES (?, 'create', 'school_years', ?, ?)");
            $stmt->execute([$_SESSION['user_id'], $yearId, $_SERVER['REMOTE_ADDR']]);
            
            $db->commit();
            setFlashMessage('School year added successfully', 'success');
            ...
            <div class="card-body">
                <form method="POST" action="">
                    <div class="row">
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <input type="text" name="year_code" class="form-control" required value="<?php echo isset($_POST['year_code']) ? htmlspecialchars($_POST['year_code']) : ''; ?>" placeholder="e.g., 2024-2025">
                        ...
                        <div class="col-md-6">
                            <div class="mb-3 form-check mt-4">
                                <input type="checkbox" name="set_active" class="form-check-input" id="setActive" <?php echo isset($_POST['set_active']) ? 'checked' : ''; ?>>
                                <label class="form-check-label" for="setActive">
                                    <strong>Set as Active School Year</strong>
                                </label>
                                <small class="d-block text-muted">This will deactivate all other school years</small>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <input type="date" name="year_start" class="form-control" required value="<?php echo isset($_POST['year_start']) ? htmlspecialchars($_POST['year_start']) : ''; ?>">
                            </div>
                        </div>
                        ...
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <input type="date" name="year_end" class="form-control" required value="<?php echo isset($_POST['year_end']) ? htmlspecialchars($_POST['year_end']) : ''; ?>">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="alert alert-info">
                        ...
                        <strong>Note:</strong> After creating the school year, you can add grading periods (quarters) from the school years list.
                    </div>
                    
                    <div class="text-end">
                        <a href="school-years.php" class="btn btn-secondary">Cancel</a>
                        <button type="submit" name="submit" class="btn btn-primary">
                            <i class="bi bi-save me-1"></i> Save School Year
                        </button>
                    </div>
                </form>
            </div>
