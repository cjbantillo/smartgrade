...
// Get student ID
$studentId = getParam('id');
if (!$studentId) {
    setFlashMessage('Invalid student ID', 'danger');
    header('Location: students.php');
    exit;
}

// Get student data
$stmt = $db->prepare("SELECT s.*, u.username, u.email as user_email, u.is_active as user_active
                      FROM students s
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!empty($lrn)) {
        $stmt = $db->prepare("SELECT COUNT(*) FROM students WHERE lrn = ? AND id != ?");
        $stmt->execute([$lrn, $studentId]);
    ...
    if (empty($errors)) {
        try {
            ...
            // Update user account
            if (!empty($password)) {
                $stmt = $db->prepare("UPDATE users SET username = ?, password = ?, email = ?, first_name = ?, last_name = ?, is_active = ? WHERE id = ?");
                $hashedPassword = password_hash($password, PASSWORD_DEFAULT);
                $stmt->execute([$username, $hashedPassword, $email, $firstName, $lastName, $isActive, $student['user_id']]);
            } else {
                $stmt = $db->prepare("UPDATE users SET username = ?, email = ?, first_name = ?, last_name = ?, is_active = ? WHERE id = ?");
                $stmt->execute([$username, $email, $firstName, $lastName, $isActive, $student['user_id']]);
            }
            
            // Update student record
            $stmt = $db->prepare("UPDATE students SET lrn = ?, first_name = ?, last_name = ?, middle_name = ?, grade_level = ?, section = ?, strand = ?, track = ?, gender = ?, birthdate = ?, email = ?, contact_number = ?, address = ?, guardian_name = ?, guardian_contact = ?, is_active = ? WHERE id = ?");
            $stmt->execute([$lrn, $firstName, $lastName, $middleName, $gradeLevel, $section, $strand, $track, $gender, $birthdate, $email, $contactNumber, $address, $guardianName, $guardianContact, $isActive, $studentId]);
            
            // Audit log
            $stmt = $db->prepare("INSERT INTO audit_logs (user_id, action, table_name, record_id, ip_address) VALUES (?, 'update', 'students', ?, ?)");
            $stmt->execute([$_SESSION['user_id'], $studentId, $_SERVER['REMOTE_ADDR']]);
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" name="lrn" class="form-control" required value="<?php echo htmlspecialchars($student['lrn'] ?? ''); ?>">
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="date" name="birthdate" class="form-control" value="<?php echo htmlspecialchars($student['birthdate'] ?? ''); ?>">
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" name="contact_number" class="form-control" value="<?php echo htmlspecialchars($student['contact_number'] ?? ''); ?>">
                            </div>
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" name="section" class="form-control" list="sectionList" required value="<?php echo htmlspecialchars($student['section'] ?? ''); ?>">
                            ...
                            <div class="mb-3">
                                ...
                                <select name="track" class="form-select">
                                    ...
                                    <option value="Academic" <?php echo ($student['track'] ?? '') == 'Academic' ? 'selected' : ''; ?>>Academic</option>
                                    <option value="TVL" <?php echo ($student['track'] ?? '') == 'TVL' ? 'selected' : ''; ?>>TVL (Technical-Vocational-Livelihood)</option>
                                    <option value="Sports" <?php echo ($student['track'] ?? '') == 'Sports' ? 'selected' : ''; ?>>Sports</option>
                                    <option value="Arts and Design" <?php echo ($student['track'] ?? '') == 'Arts and Design' ? 'selected' : ''; ?>>Arts and Design</option>
                                </select>
                            </div>
                            
                            <div class="mb-3">
                                <label class="form-label">Strand <span class="text-danger">*</span></label>
                                <input type="text" name="strand" class="form-control" list="strandList" required value="<?php echo htmlspecialchars($student['strand'] ?? ''); ?>">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" name="username" class="form-control" required value="<?php echo htmlspecialchars($student['username'] ?? ''); ?>">
                            </div>
                            ...
                            <div class="mb-3 form-check">
                                <input type="checkbox" name="is_active" class="form-check-input" id="isActive" <?php echo ($student['user_active'] ?? 0) ? 'checked' : ''; ?>>
